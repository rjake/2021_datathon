...
- Most of these cases (75%) were "ROR" (Release on Own Recognizance).  Defendants were released with no bail (there could be some conditions placed).
- 22% of cases were "Nonmonetary" for the bail type.  This means nonmonetary conditions were applied to their release.  Nonmonetary conditions can also be applied on top of monetary bail.  
- When there is an NA value for bail type this means the bail was denied.  All the action_type_name values were "Denied" when there was an NA for bail type.  
...
bail_zero <- bail %>% 
  ...
  group_by(docket_id) %>% summarise(
    bail_type = first(type_name),
  ) %>%group_by(bail_type) %>% summarise(
    n = length(bail_type)) %>%

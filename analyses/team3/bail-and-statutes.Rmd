...
ggplot(statute_percent %>% 
  ...
  theme(axis.text.x = element_text(angle = 45,hjust = 1,size = 10),
        ...
        axis.title.y = element_text(size = 12),
        legend.position = "bottom")
ggsave(paste0("figures/",Sys.Date(),"_Statutes-PercentDiff-PrePostKrasner.png"),height = 5,width = 6, device = "png", dpi=700)
...
ggplot(docket_confine %>% dplyr::filter(Change %in% c("Increase","Decrease")),
               aes(x=statute,y=min_bail,fill=timeframe)) +
  scale_y_log10() +
...
ggplot(docket_confine %>% dplyr::filter(Change %in% c("Increase","Decrease")),
               aes(x=statute,y=min_sentencing,fill=timeframe)) +
  scale_y_log10() +
  geom_boxplot(position = "dodge") + #geom_violin(color="black") +
  facet_grid(~Change,scales = "free",space="free") +
...
#### Trends in Black vs. White, Pre/Post Krasner
[X] Minimum bail ($) pre/post Krasner, separated by race and grade
[X] Minimum sentencing (days) pre/post Krasner, separated by race and grade

Subset dataset on misdemeanors and felonies, and combine ungraded and third-degree categories.
```{r}
require(scales)

mis.fel.subset <- c("M","M3","M2","M1","F","F3","F2","F1")
# mis.fel.subset <- c("F1")
docket_charges_bail <- docket_charges %>%
  dplyr::filter(race %in% c("Black","White"), grade %in% mis.fel.subset) %>%
  mutate(grade = factor(grade,levels = mis.fel.subset)) %>%
  drop_na(min_bail,min_sentencing)
           
docket_charges_bail$grade[docket_charges_bail$grade == "M"] <- "M3"
docket_charges_bail$grade[docket_charges_bail$grade == "F"] <- "F3"

docket_charges_bail$grade <- gsub("M3","M3/M",docket_charges_bail$grade)
docket_charges_bail$grade <- gsub("F3","F3/F",docket_charges_bail$grade)

docket_charges_bail$grade <- factor(docket_charges_bail$grade,levels = c("M3/M","M2","M1","F3/F","F2","F1"))

docket_charges_bail$timeframe <- gsub("pre-Krasner","before",docket_charges_bail$timeframe)
docket_charges_bail$timeframe <- gsub("post-Krasner","after",docket_charges_bail$timeframe)

docket_charges_bail$timeframe <- factor(docket_charges_bail$timeframe, levels = c("before","after"))
...
ggplot(docket_charges_bail,aes(x=timeframe,y=min_bail,color=race, fill=race)) +
...
ggsave(paste0("figures/",Sys.Date(),"_Bail-MandFgrade-Race-PrePostKrasner.png"),height = 3,width = 8, device = "png", dpi=700)

```

F1 minimum sentencing becomes more equitable between Black/White after Krasner takes office.
```{r}
...
# sentencing by MEAN
ggplot(docket_charges_bail,aes(x=timeframe,y=min_sentencing,color=race, fill=race)) +
  #scale_y_log10() +
  facet_wrap(~grade, nrow=1) +
  scale_color_manual(values = c("black", "gray")) +
  scale_fill_manual(values = c("black", "gray")) +
  ...
  theme_bw() +  theme(panel.grid.minor = element_blank())
ggsave(paste0("figures/",Sys.Date(),"_Sentencing-MandFgrade-Race-PrePostKrasner.png"),height = 3,width = 8, device = "png", dpi=700)
...
ggplot(docket_charges %>%
               ...
               aes(x=timeframe,y=min_bail,fill=race)) +
  scale_y_log10() +

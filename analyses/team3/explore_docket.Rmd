...
gender_plot <- gender_count %>%
  ...
  scale_x_continuous(
    breaks = c(-150000, -100000, -50000, 0, 50000),
    labels = abs_comma,
    expand = c(0, 10000)
  ) +
  ...
  geom_text(
    data = . %>% filter(race == "Black", gender == "Female"),
    aes(y = 5, x = n - 2000), fontface = 2,
    label = female, size = 3, color = "white"
  ) +
  geom_text(
    data = . %>% filter(race == "Black", gender == "Male"),
    aes(y = 5, x = n + 2000), fontface = 2,
    label = male, size = 3, color = "white"
  ) +
  labs(
    y = NULL,
    x = "Number of male/female defendants (by docket cases)"
  ) +
  scale_fill_manual(values = c("#CD79A7", "#009E73")) +
  theme(
...
ggsave("reports/figs/gender_race_total.png", gender_plot,
...
p <- tmp %>%
  ...
  geom_smooth(aes(group = gender, color = gender), fill = "grey80") +
  theme_classic() +
  xlab("Arrest year") +
  ylab("Defendant average age at arrest") +
  geom_text(
    data = data.frame(x = 2000.3, y = 33.7),
    aes(x = x, y = y), fontface = 2,
    label = female, size = 3, color = "black"
  ) +
  geom_text(
    data = data.frame(x = 2000.3, y = 27.7),
    aes(x = x, y = y), fontface = 2,
    label = male, size = 3, color = "black"
  ) +
  scale_x_continuous(expand = expansion(0, c(0, 0.5))) +
  scale_color_manual(values = c("#CD79A7", "#009E73")) +
  theme(legend.position = "None") +
  NULL

# p
ggsave("reports/figs/defendant_age_by_year.png", p, width = 6, height = 3)
```
...
offense <- vroom::vroom(
  ...
  col_select = c("docket_id", "grade", "sentence_type", "min_period", "max_period")
) %>%
  distinct() %>%
